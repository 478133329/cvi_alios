...
#solution目录
SDK_RELEASE_PATH=${TOP_DIR}/../${BOARD_NAME}_${SOLUTION_NAME}${PROJECT_NAME}_cvitek_sdk
#板端配置
...
release_pre_pack()
{
	...
	ls ./scripts | grep -v defconfig |xargs -i rm -rf ./scripts/{}
...
release_solutions()
{
	...
	ls ./customization/ | grep -wv "${CUSTOMIZATION_DIR}\|include" | xargs -i rm -rf ./customization/{}
	rm -f prebuild.sh
	popd
}
...
pack_release_sdk()
{
	echo "run ${FUNCNAME[0]}() function. "
	pushd ${TOP_DIR}
	tar -zcvf ../${BOARD_NAME}_${SOLUTION_NAME}${PROJECT_NAME}_cvitek_sdk.tar.gz ${SDK_RELEASE_PATH}
	rm -rf ${SDK_RELEASE_PATH}
	popd
...
pack_release_sdk
pushd ${TOP_DIR}/..
tar -xvf ${BOARD_NAME}_${SOLUTION_NAME}${PROJECT_NAME}_cvitek_sdk.tar.gz
	pushd ${BOARD_NAME}_${SOLUTION_NAME}${PROJECT_NAME}_cvitek_sdk
	echo y | yoc init
	make clean PROJECT=${PROJECT_NAME};make ${SOLUTION_NAME} PROJECT=${PROJECT_NAME}
	if [ $? != 0 ]; then
		popd
		rm -rf ${BOARD_NAME}_${SOLUTION_NAME}${PROJECT_NAME}_cvitek_sdk
		echo "release sdk build err"
		exit 1
	fi
	make clean PROJECT=${PROJECT_NAME}
	popd
mv ${BOARD_NAME}_${SOLUTION_NAME}${PROJECT_NAME}_cvitek_sdk cvi_alios_source

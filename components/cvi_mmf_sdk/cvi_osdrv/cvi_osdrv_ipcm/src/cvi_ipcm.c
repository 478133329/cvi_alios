...
CVI_S32 CVI_IPCM_GetBuffPos(CVI_U32 u32Size)
{
    return ipcm_get_buff_to_pos(u32Size);
}

CVI_S32 CVI_IPCM_ReleaseBuffByPos(CVI_U32 u32Pos)
{
    return ipcm_release_buff_by_pos(u32Pos);
}
...
CVI_S32 CVI_IPCM_DataPacked(CVI_VOID *pData, CVI_U32 u32Len, IPCM_MSG_DATA_S *pstMsg)
{
    MsgData stMsg;

    if (pstMsg == NULL) {
        ipcm_err("pstMsg is null.\n");
        return -EINVAL;
    }

    ipcm_data_packed(pData, u32Len, &stMsg);
    pstMsg->stMsgParam.stMsgPtr.data_pos = stMsg.msg_param.msg_ptr.data_pos;
    pstMsg->stMsgParam.stMsgPtr.remaining_rd_len = stMsg.msg_param.msg_ptr.remaining_rd_len;

    return 0;
}

CVI_VOID *CVI_IPCM_GetMsgData(IPCM_MSG_DATA_S *pstMsg)
{
    MsgData stMsg;
    ...
    stMsg.port_id = pstMsg->u8PortID;
    stMsg.msg_id = pstMsg->u8MsgID;
    stMsg.func_type = pstMsg->u8DataType;
    memcpy(&stMsg.resv, &pstMsg->resv, sizeof(resv_t));
    memcpy(&stMsg.msg_param, &pstMsg->stMsgParam, sizeof(MsgParam));

    return ipcm_get_msg_data(&stMsg);
}

CVI_VOID *CVI_IPCM_GetDataByPos(CVI_U32 u32DataPos)
{
    return ipcm_get_data_by_pos(u32DataPos);
}

CVI_S32 CVI_IPCM_SendMsg(IPCM_MSG_DATA_S *pstMsg)
{
    MsgData stMsg;
    ...
    return ipcm_common_send_msg(&stMsg);
}
...
CVI_S32 CVI_IPCM_AnonSendMsg(IPCM_MSG_DATA_S *pstMsg)
{
    MsgData stMsg;
    ...
    return ipcm_anon_send_msg(&stMsg);
}
...
CVI_S32 CVI_IPCM_GetRtosBootStatus(IPCM_RTOS_BOOT_STATUS_E *pBootStatus)
{
    s32 ret;
    u32 stat;

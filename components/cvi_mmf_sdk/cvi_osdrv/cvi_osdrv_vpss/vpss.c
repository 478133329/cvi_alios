...
static pthread_mutex_t vpss_reg_lock;
...
CVI_VOID vpss_notify_wkup_evt(CVI_U8 u8VpssDev)
{
	...
	aos_event_set(&handler_ctx[u8VpssDev].wait, CTX_EVENT_WKUP, AOS_EVENT_OR);
}
...
CVI_VOID vpss_notify_isr_evt(CVI_U8 img_idx)
{
	...
	for (i = 0; i < VPSS_IP_NUM; i++) {
		if (handler_ctx[i].enHdlState == HANDLER_STATE_RUN &&
			...
			aos_event_set(&handler_ctx[i].wait, CTX_EVENT_EOF, AOS_EVENT_OR);
			break;
		} else
			CVI_TRACE_VPSS(CVI_DBG_DEBUG, "handler_ctx[%d] state=%d, img_idx=%d, event=0x%x\n",
					i, handler_ctx[i].enHdlState,
...
CVI_VOID vpss_notify_vi_err_evt(CVI_U8 img_idx)
{
	...
	aos_event_set(&ctx->wait, CTX_EVENT_VI_ERR, AOS_EVENT_OR);
...
CVI_VOID vpss_gdc_callback(CVI_VOID *pParam, VB_BLK blk)
{
	...
	pthread_mutex_unlock(&mesh[_pParam->chn.s32DevId][_pParam->chn.s32ChnId].lock);
...
static VPSS_GRP findNextEnGrp(VPSS_GRP workingGrp, CVI_U8 u8VpssDev)
{
	...
	do {
		...
		if (vpssCtx[i] && vpssCtx[i]->isCreated && vpssCtx[i]->isStarted
			...
			if (jobs->inited && !sem_trywait(&jobs->sem))
				return i;
		}
...
static CVI_VOID _vpss_fill_cvi_buffer(MMF_CHN_S chn, struct vb_s *grp_vb_in, uint64_t phy_addr, struct cvi_buffer *buf, struct cvi_vpss_ctx *ctx)
{
	SIZE_S size;
...
static CVI_S32 vpss_qbuf(MMF_CHN_S chn, struct vb_s *grp_vb_in, VB_BLK chn_vb_blk, struct cvi_vpss_ctx *ctx)
{
	...
	if (vb_qbuf(chn, CHN_TYPE_OUT, blk) == CVI_SUCCESS) {
		...
		vb = (struct vb_s *)blk;
		buf = &vb->buf;
		chn_sc.s32ChnId = ctx->stChnCfgs[chn.s32ChnId].dev_idx;
		vpss_sc_qbuf(vip_dev, buf, chn_sc);
	} else
...
static CVI_S32 vpss_online_qbuf(MMF_CHN_S chn)
{
	...
	if (blk == VB_INVALID_HANDLE) {
		CVI_TRACE_VPSS(CVI_DBG_INFO, "Grp(%d) Chn(%d) online acquire VB for vpss failed\n"
			, chn.s32DevId, chn.s32ChnId);
		return CVI_FAILURE;
	}
...
static CVI_S32 vpss_sb_qbuf(struct cvi_vpss_ctx *ctx, MMF_CHN_S chn, CVI_BOOL sb_vc_ready)
{
	...
	chn_sc.s32ChnId = ctx->stChnCfgs[chn.s32ChnId].dev_idx;
	vpss_sc_sb_qbuf(vip_dev, &buf, chn_sc);
	vpss_sc_set_vc_sbm(vip_dev, chn_sc, sb_vc_ready);
...
static CVI_S32 fill_buffers(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx, bool online_from_isp)
{
	...
	for (VpssChn = 0; VpssChn < ctx->chnNum; ++VpssChn) {
		...
		if (!base_mod_jobs_waitq_empty(chn, CHN_TYPE_OUT)) {
			...
			// Implement cvi_qbuf in user space
			chn.s32ChnId = ctx->stChnCfgs[VpssChn].dev_idx;
...
{
	struct vb_s *p;
	VB_BLK blk_tmp = VB_INVALID_HANDLE;
	...
	*blk = VB_INVALID_HANDLE;
	...
	blk_tmp = base_mod_jobs_workq_pop(chn, chn_type);
	if (blk_tmp == VB_INVALID_HANDLE)
		return CVI_FAILURE;

	*blk = blk_tmp;
	p = (struct vb_s *)blk_tmp;
...
static void release_online_sc_buffers(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx)
{
	...
	for (VpssChn = 0; VpssChn < ctx->chnNum; ++VpssChn) {
		...
		if (pstChnCfg->stBufWrap.bEnable)
			continue;
		dev_idx = ctx->stChnCfgs[VpssChn].dev_idx;
...
static CVI_S32 hw_start(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx)
{
	CVI_S32 dev_idx;
	CVI_S32 s32Ret = CVI_SUCCESS;
	VPSS_CHN VpssChn;

	CVI_TRACE_VPSS(CVI_DBG_INFO, "Grp(%d), img(%d)\n", VpssGrp, ctx->u8DevId);
...
static CVI_S32 hw_stop(VPSS_GRP VpssGrp, struct cvi_vpss_ctx *ctx)
{
	CVI_S32 dev_idx;
	CVI_S32 s32Ret = CVI_SUCCESS;
	VPSS_CHN VpssChn;
	...
	struct VPSS_GRP_HW_CFG *pstGrpHwCfg, bool online_from_isp)
{
	...
	if (stChnCfg->stChnAttr.stNormalize.bEnable) {
		...
		for (CVI_U8 i = 0; i < 3; ++i) {
			pstHwCfg->quantCfg.sc_frac[i] = stChnCfg->stChnAttr.stNormalize.factor[i] * 8192;
			pstHwCfg->quantCfg.sub_frac[i]
				= modf(stChnCfg->stChnAttr.stNormalize.mean[i], &sub_int) * 1024;
			pstHwCfg->quantCfg.sub[i] = sub_int;
		}
...
static CVI_S32 commitHWSettings(VPSS_GRP VpssGrp, bool online_from_isp, struct cvi_vpss_ctx *ctx)
{
	...
	if (!online_from_isp && _is_frame_crop_changed(VpssGrp, ctx))
...
static void _update_vpss_frame_rate(void *timer, void *arg)
{
	int i, j;
...
static CVI_U64 get_diff_time_us(uint64_t t1, uint64_t t2)
{
	CVI_U64 diff_time;
...
static CVI_S32 vpss_try_schedule_online(struct vpss_handler_ctx *ctx)
{
	...
	for (workingGrp = 0; workingGrp < VPSS_ONLINE_NUM; workingGrp++) {
		...
		if (!(vpssExtCtx[workingGrp].grp_state & GRP_STATE_HW_STARTED)) {
			...
			if (vc_sbm_enabled) {
				...
				if (_notify_vc_sb_mode(workingGrp, vpss_ctx, VCODEC_CB_SEND_FRM, CVI_TRUE) != CVI_SUCCESS) {
					...
					hw_start(workingGrp, vpss_ctx);
					break;
				}
				fill_sb_buffers(workingGrp, vpss_ctx, true);
			}

			if (hw_start(workingGrp, vpss_ctx) != CVI_SUCCESS) {
		...
		ctx->enHdlState = HANDLER_STATE_RUN;
		pthread_mutex_unlock(&vpssCtx[workingGrp]->lock);
...
static CVI_VOID vpss_handle_online_frame_done(struct vpss_handler_ctx *ctx, VPSS_GRP workingGrp)
{
	...
	if (workingMask == 0)
		ctx->enHdlState = HANDLER_STATE_READY;
	// job done.
...
static CVI_VOID vpss_handle_online(struct vpss_handler_ctx *ctx)
{
	...
	if (events & CTX_EVENT_VI_ERR) {
		...
		vip_dev->img_vdev[dev_idx].overflow_cnt++;
...
static CVI_S32 vpss_try_schedule_offline(struct vpss_handler_ctx *ctx)
{
	...
	CVI_TRACE_VPSS(CVI_DBG_DEBUG, "ctx[%d] workingGrp=%d, workingMask=%d\n",
			ctx->u8VpssDev, workingGrp, workingMask);
...
vpss_next_job:
	...
	ctx->workingMask = 0;
	ctx->enHdlState = HANDLER_STATE_READY;
	if (hw_stop(workingGrp, vpss_ctx) != CVI_SUCCESS)
...
static void vpss_handle_offline(struct vpss_handler_ctx *ctx)
{
	...
	if (ctx->enHdlState == HANDLER_STATE_READY) {
next_grp:
		...
		if (workingGrp == VPSS_MAX_GRP_NUM)
			return;
		if (vpss_try_schedule_offline(ctx)) {
			goto next_grp;
		}
	} else if (ctx->enHdlState == HANDLER_STATE_RUN) {
		spin_lock_irqsave(&ctx->lock, flags_job);
		events = ctx->events;
		spin_unlock_irqrestore(&ctx->lock, flags_job);

		CVI_TRACE_VPSS(CVI_DBG_DEBUG, "ctx[%d] event=0x%x, Grp(%d) mask=0x%x\n",
				ctx->u8VpssDev, events, workingGrp, ctx->workingMask);
		if (events & CTX_EVENT_EOF) {
			vpss_handle_frame_done(ctx);
			spin_lock_irqsave(&ctx->lock, flags_job);
			ctx->events = 0;
			spin_unlock_irqrestore(&ctx->lock, flags_job);
			aos_event_set(&ctx->wait, CTX_EVENT_WKUP, AOS_EVENT_OR);
		}
	}
}

void vpss_handler_timeout(struct vpss_handler_ctx *ctx)
{
	uint64_t time_us;
	CVI_U64 duration64_us;
	unsigned long flags_job;
	VPSS_GRP workingGrp = VPSS_MAX_GRP_NUM;
	...
	if ((duration64_us < EOF_WAIT_TIMEOUT_MS) ||
		(workingGrp >= VPSS_MAX_GRP_NUM))
		return;

	if (ctx->online_from_isp) {
		if (duration64_us > EOF_WAIT_TIMEOUT_MS && duration64_us < (2 * EOF_WAIT_TIMEOUT_MS)) {
			CVI_TRACE_VPSS(CVI_DBG_NOTICE, "ctx[%d] online event timeout on.\n", ctx->u8VpssDev);
			sclr_check_register();
		}
	} else {
		CVI_TRACE_VPSS(CVI_DBG_ERR, "ctx[%d] event timeout on grp(%d) time(ms):%d\n",
				ctx->u8VpssDev, workingGrp, duration64_us);
		sclr_check_register();
		spin_lock_irqsave(&ctx->lock, flags_job);
		ctx->events = 0;
		ctx->workingGrp = VPSS_MAX_GRP_NUM;
...
void *vpss_event_handler(void *arg)
{
	...
	CVI_U64 timeout = EOF_WAIT_TIMEOUT_MS;
	unsigned int actl_flags;
	int ret = 0;

	while (!ctx->bStop) {
		timeout = (ctx->enHdlState == HANDLER_STATE_RUN) ? EOF_WAIT_TIMEOUT_MS : IDLE_TIMEOUT_MS;

		ret = aos_event_get(&ctx->wait, CTX_EVENT_WKUP | CTX_EVENT_EOF | CTX_EVENT_VI_ERR,
				AOS_EVENT_OR_CLEAR, &actl_flags, timeout);

		/* timeout */
		if (ret < 0) {
			if (ctx->enHdlState == HANDLER_STATE_RUN)
				vpss_handler_timeout(ctx);
			continue;
		}
	...
	if (ctx->enHdlState == HANDLER_STATE_RUN)
		hw_reset(ctx->workingGrp, &ctx->vpssCtxWork[0], CVI_FALSE);

	CVI_TRACE_VPSS(CVI_DBG_NOTICE, "thread end, u8VpssDev=%d\n", ctx->u8VpssDev);

	return NULL;
}

static CVI_S32 vpss_start_handler(CVI_U8 u8VpssDev)
{
	CVI_S32 ret;
	...
	pthread_setname_np(handler_ctx[u8VpssDev].v_thread, thread_name);
	//FIFO_INIT(&handler_ctx[u8VpssDev].rgnex_jobs.jobq, 16);
	CVI_TRACE_VPSS(CVI_DBG_NOTICE, "thread start, u8VpssDev=%d\n", u8VpssDev);

	return ret;
}

static CVI_S32 vpss_stop_handler(CVI_U8 u8VpssDev)
{
	int ret;
	...
	handler_ctx[u8VpssDev].bStop = true;
	aos_event_set(&handler_ctx[u8VpssDev].wait, CTX_EVENT_WKUP, AOS_EVENT_OR);

	ret = pthread_join(handler_ctx[u8VpssDev].v_thread, NULL);
	if (ret)
		CVI_TRACE_VPSS(CVI_DBG_ERR, "fail to stop vpss thread, err=%d\n", ret);

	handler_ctx[u8VpssDev].v_thread = NULL;
	handler_ctx[u8VpssDev].enHdlState = HANDLER_STATE_STOP;
	aos_event_free(&handler_ctx[u8VpssDev].wait);
	sem_destroy(&handler_ctx[u8VpssDev].vi_reset_wait);
	spin_lock_destroy(&handler_ctx[u8VpssDev].lock);
	CVI_TRACE_VPSS(CVI_DBG_NOTICE, "thread stop, u8VpssDev=%d\n", u8VpssDev);
...
void _os_vpss_GrpParamInit(VPSS_GRP VpssGrp)
{
	...
	for (i = 0; i < vpssCtx[VpssGrp]->chnNum; ++i) {
		...
		chn.s32ChnId = i;
		vpssCtx[VpssGrp]->stChnCfgs[i].dev_idx = get_dev_info_by_chn(chn, CHN_TYPE_OUT);
...
CVI_S32 _os_vpss_create_grp(VPSS_GRP VpssGrp, const VPSS_GRP_ATTR_S *pstGrpAttr)
{
	...
	for (VpssChn = 0; VpssChn < vpssCtx[VpssGrp]->chnNum; ++VpssChn) {
		pthread_mutex_init(&mesh[VpssGrp][VpssChn].lock,NULL);
	}
...
CVI_S32 _os_vpss_stop_grp(VPSS_GRP VpssGrp)
{
	...
	if (!enabled && state != HANDLER_STATE_STOP)
		vpss_stop_handler(u8VpssDev);
...
CVI_S32 _os_vpss_set_chn_attr(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, const VPSS_CHN_ATTR_S *pstChnAttr)
{
	...
	if (vpssCtx[VpssGrp]->stChnCfgs[VpssChn].VbPool == VB_INVALID_POOLID) {
		...
		_find_vb_pool(stVbCalConfig.u32VBSize);
	}
...
CVI_S32 _os_vpss_enable_chn(VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
	...
	u8VpssDev = vpssCtx[VpssGrp]->u8DevId;
	spin_unlock(&vpssCtx[VpssGrp]->lock);
...
CVI_S32 _os_vpss_send_frame(VPSS_GRP VpssGrp, const VIDEO_FRAME_INFO_S *pstVideoFrame, CVI_S32 s32MilliSec)
{
	...
	CVI_TRACE_VPSS(CVI_DBG_DEBUG, "Grp(%d), phy-address(0x%lx)\n",
			VpssGrp, pstVideoFrame->stVFrame.u64PhyAddr[0]);

	return CVI_SUCCESS;
}

CVI_S32 _os_vpss_send_chn_frame(VPSS_GRP VpssGrp, VPSS_CHN VpssChn
			...
			CVI_S32 s32MilliSec)
{
	...
	CVI_TRACE_VPSS(CVI_DBG_DEBUG, "Grp(%d) Chn(%d) end to set pstFrameInfo width:%d height:%d buf:0x%lx\n"
...
CVI_S32 _os_vpss_release_chn_frame(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, const VIDEO_FRAME_INFO_S *pstVideoFrame)
{
	...
	CVI_TRACE_VPSS(CVI_DBG_DEBUG, "Grp(%d) Chn(%d) buf:0x%lx\n",
...
CVI_S32 _os_vpss_detach_vb_pool(VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
	...
	vpssCtx[VpssGrp]->stChnCfgs[VpssChn].stChnWorkStatus.PoolId =
		_find_vb_pool(vpssCtx[VpssGrp]->stChnCfgs[VpssChn].blk_size);
	pthread_mutex_unlock(&vpssCtx[VpssGrp]->lock);
	CVI_TRACE_VPSS(CVI_DBG_DEBUG, "Grp(%d) Chn(%d)\n", VpssGrp, VpssChn);
...
void vpss_mode_init(struct cvi_vip_dev *pdev)
{
	...
	for (i = 0; i < VPSS_IP_NUM; ++i) {
		...
		handler_ctx[i].enHdlState = HANDLER_STATE_STOP;
	}
	vpss_set_mode_ex(pdev, &stVPSSMode);
}

static aos_timer_t vpss_proc_timer;
...
void _os_vpss_init(struct cvi_vip_dev *pdev)
{
	...
	pthread_mutex_init(&vpss_reg_lock, NULL);   //for suspend and resume

	aos_timer_new(&vpss_proc_timer, _update_vpss_frame_rate, NULL, 1000, AOS_TIMER_REPEAT);
	aos_timer_start(&vpss_proc_timer);
}

void vpss_deinit(void)
{
	if (aos_timer_is_valid(&vpss_proc_timer)) {
		aos_timer_stop(&vpss_proc_timer);
		aos_timer_free(&vpss_proc_timer);
	}

	pthread_mutex_destroy(&vip_dev->wait_lock);
	pthread_mutex_destroy(&vip_dev->reset_lock);
	pthread_mutex_destroy(&vpss_reg_lock);
...
int vpss_set_vc_sbm_flow(struct vpss_vc_sbm_flow_cfg *cfg)
{
	...
	chn.s32ChnId = vpssCtx[VpssGrp]->stChnCfgs[VpssChn].dev_idx;
...
CVI_S32 vpss_set_rgn_lut_cfg(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, struct cvi_rgn_lut_cfg *cfg)
{
	...
	dev_idx = vpssCtx[VpssGrp]->stChnCfgs[VpssChn].dev_idx;
	pthread_mutex_lock(&vpssCtx[VpssGrp]->lock);
	if (cfg->rgnex_en)
...
CVI_S32 vpss_overflow_check(VPSS_GRP VpssGrp, struct cvi_vpss_info *vpss_info, struct cvi_vc_info *vc_info)
{
	...
	dev_idx = vpssCtx[VpssGrp]->stGrpAttr.u8VpssDev;
	...
	if (handler_ctx[dev_idx].online_from_isp) {
		...
		//CVI_TRACE_VPSS(CVI_DBG_ERR, "overflow check reg\n");
	}
...
CVI_S32 _os_vpss_suspend(void)
{
	...
	for (i = 0; i < VPSS_IP_NUM; ++i) {
		u8VpssDev = i;
		s32Ret = vpss_stop_handler(u8VpssDev);
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_VPSS(CVI_DBG_ERR, "fail to stop vpss thread, err=%d\n", s32Ret);
			return s32Ret;
		} else
			CVI_TRACE_VPSS(CVI_DBG_INFO, "thread stopped, u8VpssDev=%d\n", u8VpssDev);
	}

	/*step 2 turn off clock*/
	pthread_mutex_lock(&vpss_reg_lock);
	sclr_clk_disable();
	pthread_mutex_unlock(&vpss_reg_lock);

	CVI_TRACE_VPSS(CVI_DBG_INFO, "vpss suspended\n");
	return CVI_SUCCESS;
}

CVI_S32 _os_vpss_resume(void)
{
	CVI_S32 s32Ret = CVI_SUCCESS;
	CVI_U8 u8VpssDev = 0;
	CVI_U32 i = 0;
	...
	/*step 2 start all envent_handlers*/
	for (i = 0; i < VPSS_IP_NUM; ++i) {
		u8VpssDev = i;
		s32Ret = vpss_start_handler(u8VpssDev);
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_VPSS(CVI_DBG_ERR, "fail to start vpss thread, err=%d\n", s32Ret);
			return s32Ret;
		} else
			CVI_TRACE_VPSS(CVI_DBG_ERR, "thread started, u8VpssDev=%d\n", u8VpssDev);
	}

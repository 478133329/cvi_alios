...
void cvi_sc_device_run(void *priv, bool is_tile, bool is_work_on_r_tile, u8 grp_id)
{
	...
	if (sdev->sb_enabled && sdev->sb_phy_addr[0]) {
		b = &sb_buf;
		for (i = 0; i < 3; i++)
			b->planes[i].addr = sdev->sb_phy_addr[i];
	} else {
...
#endif
	sdev->sb_enabled = false;
	sdev->sb_vc_ready = false;
...
void cvi_sc_update(struct cvi_sc_vdev *sdev, const struct cvi_vpss_chn_cfg *chn_cfg)
{
	...
	if (chn_cfg->bind_fb) {
		...
		if (disp_cfg->ow_cfg[0].fmt == 0)
			disp_cfg->ow_cfg[0].fmt = SCL_GOP_FMT_ARGB8888;
		else if (disp_cfg->ow_cfg[0].fmt == 0x4)
			disp_cfg->ow_cfg[0].fmt = SCL_GOP_FMT_ARGB4444;
		else if (disp_cfg->ow_cfg[0].fmt == 0x5)
			disp_cfg->ow_cfg[0].fmt = SCL_GOP_FMT_ARGB1555;
		else if (disp_cfg->ow_cfg[0].fmt == 0x8)
			disp_cfg->ow_cfg[0].fmt = SCL_GOP_FMT_256LUT;
		else if (disp_cfg->ow_cfg[0].fmt == 0xa)
			disp_cfg->ow_cfg[0].fmt = SCL_GOP_FMT_16LUT;
		else if (disp_cfg->ow_cfg[0].fmt == 0xc)
			disp_cfg->ow_cfg[0].fmt = SCL_GOP_FMT_FONT;
		sclr_gop_ow_set_cfg(sdev->dev_idx, 0, 0, &disp_cfg->ow_cfg[0], true);
		if (cfg.ow_cfg[0].addr)
			cfg.gop_ctrl.b.ow0_en = true;
		else
			cfg.gop_ctrl.b.ow0_en = false;
		...
		for (i = 0; i < SCL_MAX_GOP_INST; ++i) {
			...
			if (rgn_cfg->odec.enable) {
				...
				CVI_TRACE_VPSS(CVI_DBG_DEBUG,
					"ow%d:%11s(%4d)%10s(%4d %4d %4d %4d)%10s(%6d)%10s(0x0x%llx)\n", ow_idx,
	...
	sclr_set_sclr_to_vc_sb(sdev->dev_idx, chn_cfg->sb_cfg.sb_mode, chn_cfg->sb_cfg.sb_size,
				chn_cfg->sb_cfg.sb_nb, chn_cfg->sb_cfg.sb_wr_ctrl_idx);
...
CVI_VOID vpss_sc_sb_qbuf(struct cvi_vip_dev *dev, struct cvi_buffer *buf, MMF_CHN_S chn)
{
	...
	if (buf) {
		...
		sdev->sb_enabled = true;

		for (i = 0; i < 3; i++)
			sdev->sb_phy_addr[i] = buf->phy_addr[i];
	} else {
		sdev->sb_enabled = false;
		sdev->sb_vc_ready = false;
		for (i = 0; i < 3; i++)
			sdev->sb_phy_addr[i] = 0;
	}
}

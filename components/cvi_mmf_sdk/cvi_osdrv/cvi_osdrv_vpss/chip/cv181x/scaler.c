...
extern void csi_dcache_clean_range (uint64_t *addr, int64_t dsize);
...
struct sclr_gop_cfg *sclr_gop_get_cfg(u8 inst, u8 layer)
{
	...
	if (inst < SCL_MAX_INST && layer < SCL_MAX_GOP_INST)
		...
		for (i = 0; i < SCL_MAX_GOP_OW_INST; ++i) {
			gop_ow_cfg.fmt = _reg_read(reg_base + REG_SCL_DISP_GOP_FMT(i)) & 0x0F;
			gop_ow_cfg.end.x = (_reg_read(reg_base + REG_SCL_DISP_GOP_H_RANGE(i)) >> 16) & 0xFFF;
			gop_ow_cfg.start.x = _reg_read(reg_base + REG_SCL_DISP_GOP_H_RANGE(i)) & 0xFFF;
			gop_ow_cfg.end.y = (_reg_read(reg_base + REG_SCL_DISP_GOP_V_RANGE(i)) >> 16) & 0xFFF;
			gop_ow_cfg.start.y = _reg_read(reg_base + REG_SCL_DISP_GOP_V_RANGE(i)) & 0xFFF;
			gop_ow_cfg.addr = _reg_read(reg_base + REG_SCL_DISP_GOP_ADDR_L(i)) |
						(u64)_reg_read(reg_base + REG_SCL_DISP_GOP_ADDR_H(i)) << 32;
			gop_ow_cfg.pitch = _reg_read(reg_base + REG_SCL_DISP_GOP_CROP_PITCH(i)) & 0xFFFF;
			gop_ow_cfg.crop_pixels = (_reg_read(reg_base + REG_SCL_DISP_GOP_CROP_PITCH(i)) >> 16) & 0x7F;
			gop_ow_cfg.mem_size.w = _reg_read(reg_base + REG_SCL_DISP_GOP_SIZE(i)) & 0x3FFF;
			gop_ow_cfg.mem_size.h = (_reg_read(reg_base + REG_SCL_DISP_GOP_SIZE(i)) >> 16) & 0xFFF;

			g_disp_cfg.gop_cfg.ow_cfg[i] = gop_ow_cfg;
		}
		g_disp_cfg.gop_cfg.gop_ctrl.raw = _reg_read(reg_base + REG_SCL_DISP_GOP_CFG);
		g_disp_cfg.gop_cfg.font_bg_color = _reg_read(reg_base + REG_SCL_DISP_GOP_FONTCOLOR) & 0xFFFF;
		g_disp_cfg.gop_cfg.font_fg_color = (_reg_read(reg_base + REG_SCL_DISP_GOP_FONTCOLOR) >> 16) & 0xFFFF;
		g_disp_cfg.gop_cfg.fb_ctrl.raw = _reg_read(reg_base + REG_SCL_DISP_GOP_FONTBOX_CTRL);
		g_disp_cfg.gop_cfg.fb_ctrl.b.lo_thr_inv = _reg_read(reg_base + 0x90f8) & 0x01;
		g_disp_cfg.gop_cfg.odec_cfg.odec_ctrl.raw = _reg_read(reg_base + REG_SCL_DISP_GOP_DEC_CTRL);
...
/**
 * sclr_disp_get_crop_cfg_from_reg - get crop settings from register.
 *
 */
void sclr_disp_get_crop_cfg_from_reg(void)
{
	u32 tmp = 0;

	tmp = _reg_read(reg_base + REG_SCL_DISP_OFFSET);
	g_disp_cfg.mem.start_x = tmp & 0xffff;
	g_disp_cfg.mem.start_y = (tmp >> 16) & 0xffff;
}
...
#endif
#endif
	csi_dcache_clean_range((uintptr_t *)cmdq_addr,0x2000);
...
static void _gop_tile_shift(struct sclr_gop_ow_cfg *gop_ow_cfg, u16 left_width, u16 total_width, bool is_right_tile)
{
	...
	CVI_TRACE_VPSS(CVI_DBG_ERR, "gop_tile(%s): addr(%#llx), startx(%d), endx(%d) img_width(%d) mem_width(%d)\n",
...
{
	void __iomem *vip_base = (void __iomem *)0x0A0C8070;
...
{
	void __iomem *ddr_base = (void __iomem *)0x08004544;;

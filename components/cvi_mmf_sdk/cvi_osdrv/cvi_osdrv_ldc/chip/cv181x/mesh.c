			  ...
			  ROTATION_E enRotation)
{
	...
	_pcbParam = aos_malloc(cbParamSize);
	if (!_pcbParam) {
			   ...
			   ROTATION_E enRotation)
{
	...
	pstTask = aos_malloc(sizeof(*pstTask));
	if (!pstTask) {
			   ...
			   ROTATION_E enRotation)
{
	...
	pstTask[0] = aos_malloc(sizeof(struct gdc_task_attr));
	pstTask[1] = aos_malloc(sizeof(struct gdc_task_attr));
	...
	ret = init_ldc_param(vb_in, vb_out[0], pstTask[0], enPixFormat,
			     ...
			     enRotationOut[0]);
	if (ret) {
		vb_out[0]->mod_ids &= ~BIT(CVI_ID_GDC);
		vb_release_block((VB_BLK)vb_out[0]);
	...
	if (ret) {
		vb_out[1]->mod_ids &= ~BIT(CVI_ID_GDC);
		vb_release_block((VB_BLK)vb_out[1]);
		CVI_TRACE_LDC(CVI_DBG_ERR, "init 2nd ldc param failed\n");
		aos_free((void *)(uintptr_t)pstTask[0]->au64privateData[2]);
		goto LDC_FAIL_EXIT;
	}

...
void rcGopBitAlloc(stRcKernelInfo *info, int picIdx)
{
	...
	info->gopPicLeft = info->isLastPicI ? (gopSize - 1) : gopSize;
	info->gopBitLeft = picTargetBit * gopSize;

	// printf("mark idx:%d, gopSize:%d ", picIdx, info->gopPicLeft);
...
int estPicTargetBits(stRcKernelInfo *info, int rcGopFirstFrame)
{
...
RC_Float estPicLambda(stRcKernelInfo *info, int targetBits, int isIPic, int picIdx)
{
	...
	soft_float_do_div(&targetBits64, info->numOfPixel);
...
void cviRcKernel_init(stRcKernelInfo *info, stRcKernelCfg *cfg)
{
	...
	info->bitrateChange = 0;
	info->rcGopPicWeight = cfg->rcGopPicWeight;	// cbr:1, avbr:3
	info->sceneChange = 0;
	info->bitIvalid = 0;
}
...
void cviRcKernel_estimatePic(stRcKernelInfo *info, stRcKernelPicOut *out, int isIPic, int picIdx)
{
	...
	if ((isIPic && info->intraPeriod != 1) || info->isFirstPic == 1) {
		...
		rcGopFrameIdx = ((picIdx % info->statFrameNum) % info->rcGopSize);
...
void cviRcKernel_updatePic(stRcKernelInfo *info, stRcKernelPicIn *stats, int isIPic)
{
	...
	if (isIPic == 1) {
		...
		if (info->intraPeriod != 1) {
			...
			info->predictPicAvgBit = info->statBitrate;
		}
	...
	if (info->rcGopSize > 1 && stats->encodedBit >= (info->rcGopSize*info->picTargetBit)) {
		info->bitIvalid = 1;
	}

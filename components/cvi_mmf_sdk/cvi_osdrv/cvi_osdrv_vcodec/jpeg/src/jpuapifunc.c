...
jpg_info_t jpginfo = {0};
#else
static SemaphoreHandle_t jpgLock = NULL;
#endif
...
static int wraparound_bistream_data(JpgDecInfo *jpg, int return_type)
{
	...
	src = (BYTE *)calloc(1, src_size);
	dst_size = ((src_size + (JPU_GBU_SIZE - 1)) & ~(JPU_GBU_SIZE - 1));
	dst = (BYTE *)calloc(1, dst_size);
	if (dst && src) {
...
#ifdef USE_POSIX
	int ret = 0;
	jpg_info_t *jpg = &jpginfo;

	if(jpg->jpg_chn_num == 0) {
		ret = sem_init(&jpg->jpgSem, 0, 1);
		if(ret != 0)
			return JPG_RET_FAILURE;
	}
...
JpgRet JpgEnterTryLock(void)
{
	...
	if (SEM_TIMEDWAIT(jpg->jpgSem, tout))
		return JPG_RET_FAILURE;
#else
	if (TRUE != xSemaphoreTake(jpgLock, 0))
		return JPG_RET_FAILURE;
#endif
...
JpgRet JpgEnterTimeLock(int timeout_ms)
{
	...
	if (SEM_TIMEDWAIT(jpg->jpgSem, tout))
		return JPG_RET_FAILURE;
#else
	if (TRUE != xSemaphoreTake(jpgLock, -1))
		return JPG_RET_FAILURE;
#endif

...
int cviJpgDecSendFrameData(CVIJpgHandle handle, void *data, int length)
{
	...
	if (pDecInfo->dst_type != JPEG_MEM_EXTERNAL) {
		...
		if (pDecInfo->rotationEnable == 1 && pDecInfo->mirrorEnable == 1) {
			JPU_DecGiveCommand(handle, SET_JPG_ROTATOR_OUTPUT, &(pDecInfo->frameBuf[0])); // bug for rotator
			JPU_DecGiveCommand(handle, SET_JPG_ROTATOR_STRIDE, &framebufStride);
		}

		// Register frame buffers requested by the decoder.
		ret = JPU_DecRegisterFrameBuffer(handle, pDecInfo->frameBuf, regFrameBufCount, framebufStride);
		if (ret != JPG_RET_SUCCESS) {
	...
	while (1) {
		...
		if (-1 == int_reason) {
			...
			if (ret < 0) {
				...
				printf("info : jpu dec interruption timeout happened and is reset automatically\n");
			}
#endif

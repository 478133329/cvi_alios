...
#define CVI_RC_PIC_WEIGHT_CBR	1
#define CVI_RC_PIC_WEIGHT_AVBR	3
...
int _cviEncRc_getBitrate(stRcInfo *pRcInfo)
{
	...
	return (pRcInfo->rcMode == RC_MODE_CBR || pRcInfo->rcMode == RC_MODE_UBR) ?
...
static void _cviEncRc_setBitrateParam(stRcInfo *pRcInfo, EncOpenParam *pEncOP)
{
	...
	if ((pRcInfo->codec == STD_AVC || pRcInfo->rcMode == RC_MODE_UBR) &&
		...
		if (pRcInfo->cviRcEn) {
			...
			pRcInfo->maxIPicBit =
				(int)(pEncOP->maxIprop *
						((float)pEncOP->gopSize * pRcInfo->targetBitrate *
						 1000 /
						 (fFramerate *
						  (pEncOP->maxIprop + pEncOP->gopSize - 1))));
#endif
	...
	if (pRcInfo->cviRcEn) {
		...
		if (dbg_mask & CVI_MASK_CVRC) {
			CVI_VCOM_FLOAT("bitRate = %d, frameRate = %f\n", pRcInfo->targetBitrate, getFloat(frameRate));
		}
...
BOOL cviEnc_Avbr_PicCtrl(stRcInfo *pRcInfo, EncOpenParam *pEncOP, int frameIdx)
{
	...
	if (pRcInfo->avbrChangeValidCnt == 0) {
		...
		int diffMotLv = (pRcInfo->lastPeriodMotionLv > pRcInfo->periodMotionLv) ?
						(pRcInfo->lastPeriodMotionLv - pRcInfo->periodMotionLv) >> 4 :
						(pRcInfo->periodMotionLv - pRcInfo->lastPeriodMotionLv) >> 4;
		changeBrEn = (diffMotLv >= 1) ||
		             (pRcInfo->periodMotionLv == 0 && pRcInfo->lastPeriodMotionLv > 0 && pRcInfo->lastPeriodMotionLv <= 16);
...
void cviEncRc_Open(stRcInfo *pRcInfo, EncOpenParam *pEncOP)
{
	...
	// frame skipping
	pRcInfo->contiSkipNum = (!pEncOP->frmLostOpen) ? 0 : (pEncOP->encFrmGaps==0) ? 65535 : pEncOP->encFrmGaps;
	...
	pRcInfo->avbrContiSkipNum = (!pEncOP->avbrFrmLostOpen) ? 0 : (pEncOP->avbrFrmGaps==0) ? 65535 : pEncOP->avbrFrmGaps;
...
static void cviEncRc_RcKernelInit(stRcInfo *pRcInfo, EncOpenParam *pEncOP)
{
	...
	pRcKerCfg->rcGopPicWeight = (pRcInfo->rcMode == RC_MODE_AVBR) ? CVI_RC_PIC_WEIGHT_AVBR : CVI_RC_PIC_WEIGHT_CBR;
...
void cviEncRc_RcKernelEstimatePic(stRcInfo *pRcInfo, EncParam *pEncParam, int frameIdx)
{
	...
	if (pRcInfo->rcMode == RC_MODE_AVBR) {
		...
		pRcPicOut->qp = CLIP3(minQp, maxQp + pRcInfo->qDelta, pRcPicOut->qp);
		pEncParam->u32FrameQp = pRcPicOut->qp;
	}
}
...
static void cviEncRc_RcKernelUpdatePic(stRcInfo *pRcInfo, EncOutputInfo *pEncOutInfo)
{
	...
	if (pRcInfo->codec == STD_AVC) {
		...
		if (pEncOutInfo->picType != 0 && pEncOutInfo->numOfIntra >= 0.5*mbs) {
		...
		int mbs = pRcInfo->numOfPixel / 1024;
	...
	if (dbg_mask & CVI_MASK_CVRC) {
		printf("%4d, %2d, %9d, %2d, %11d, %2d, %11d, %11d, %d,%d \n",
				pRcInfo->frameIdx, pEncOutInfo->picType, pRcPicOut->targetBit, pRcPicOut->qp, pRcPicIn->encodedBit,
				CVI_FLOAT_TO_INT(pRcPicIn->encodedQp), (pEncOutInfo->bitstreamSize << 3), pRcInfo->rcKerInfo.bitError,
				pEncOutInfo->numOfIntra, pEncOutInfo->numOfSkipBlock);
	}
}

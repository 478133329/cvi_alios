...
#endif
#define VDI_SRAM_SIZE_1822_CODA9        0xEC00
#define VDI_SRAM_SIZE_1822_WAVE420L     0x19400
#define VDI_SRAM_SIZE_1835_CODA9        0x1D000 // FHD MAX size, 0x17D00  4K MAX size 0x34600
#define VDI_SRAM_SIZE_1835_WAVE420L     0x1D000
...
SEM_T singleEsBuf_sem[MAX_NUM_VPU_CORE];
#else
...
static void vdi_SingleEsBuffer_initlock(unsigned long core_idx);
static void vdi_SingleEsBuffer_deinitlock(unsigned long core_idx);

#ifdef SUPPORT_INTERRUPT
extern int request_irq(unsigned int irqn, void *handler, unsigned long flags, const char *name, void *priv);

int vdi_irq_handler(int irq, void *priv)
{
	vdi_info_t *vdi = (vdi_info_t *)priv;
	int product_code = vdi_read_register(vdi->core_idx, VPU_PRODUCT_CODE_REGISTER);

	if (PRODUCT_CODE_W_SERIES(product_code)) {
		if (vdi_read_register(vdi->core_idx, W4_VPU_VPU_INT_STS)) {
			vdi->intr_reason = vdi_read_register(vdi->core_idx, W4_VPU_VINT_REASON);
			vdi_write_register(vdi->core_idx, W4_VPU_VINT_REASON_CLR, vdi->intr_reason);
			vdi_write_register(vdi->core_idx, W4_VPU_VINT_CLEAR, 0x1);
		}
	} else if (PRODUCT_CODE_NOT_W_SERIES(product_code)) {
		if (vdi_read_register(vdi->core_idx, BIT_INT_STS)) {
			vdi->intr_reason = vdi_read_register(vdi->core_idx, BIT_INT_REASON);
			vdi_write_register(vdi->core_idx, BIT_INT_CLEAR, 0x1);
		}
	}

	SEM_POST(vdi->irq_sem);
...
int allocate_common_memory(unsigned long core_idx)
{
	...
	if (vdi->vpu_common_memory.base == 0) {
		vdb.base = (Uint64)calloc(1, vdb.size + 16*1024);
		if (0 == vdb.base) {
			CVI_VC_ERR("[VDI] alloc fail, size:%d\n", vdb.size + 16*1024);
			return -1;
		}

		memset((void *)vdb.base, 0x0, vdb.size + 16*1024);
...
int vdi_open_instance(unsigned long core_idx, unsigned long inst_idx)
{
	...
	vdi->s_interrupt_flag = 0;

	if (vdi->pvip->vpu_instance_num == 0)
		SEM_INIT(vdi->irq_sem);

	request_irq(vdi->irq_num, vdi_irq_handler, 0, "VCODEC", vdi);
#endif

	if (vdi->pvip->vpu_instance_num == 0)
		vdi_SingleEsBuffer_initlock(core_idx);
...
int vdi_close_instance(unsigned long core_idx, unsigned long inst_idx)
{
	...
	if (vdi->pvip->vpu_instance_num == 0) {
#ifdef SUPPORT_INTERRUPT
		SEM_DESTROY(vdi->irq_sem);
#endif
		vdi_SingleEsBuffer_deinitlock(core_idx);
	}
...
#else
	if (TRUE != xSemaphoreTake((SemaphoreHandle_t)vdi->vpu_mutex, -1))
		return -1;
#endif
...
#else
	if (TRUE == xSemaphoreTake((SemaphoreHandle_t)vdi->vpu_mutex, 0)) {
		xSemaphoreGive((SemaphoreHandle_t)vdi->vpu_mutex);
		return -1;
	}
#endif
	return 0;
...
#else
	if (TRUE != xSemaphoreGive((SemaphoreHandle_t)vdi->vpu_mutex)) {
		CVI_VC_ERR("unlock failed\n");
	}
#endif
...
#else
	if (TRUE == xSemaphoreTake((SemaphoreHandle_t)vcodecLock[core_idx], 0))
		return 0;
...
#else
	if (TRUE != xSemaphoreTake((SemaphoreHandle_t)vcodecLock[core_idx], -1)) {
		CVI_VC_ERR("lock error\n");
		return -1;
	}
#endif
...
#else
	xSemaphoreTake((SemaphoreHandle_t)vcodecLock[core_idx], -1);
#endif
}
...
#else
	xSemaphoreGive((SemaphoreHandle_t)vcodecLock[core_idx]);
#endif
}

static void vdi_SingleEsBuffer_initlock(unsigned long core_idx)
{
	CVI_VC_LOCK("core_idx:%ld\n", core_idx);

	SEM_INIT(singleEsBuf_sem[core_idx]);

	//SEM_POST for first frame
	SEM_POST(singleEsBuf_sem[core_idx]);
}
...
static void vdi_SingleEsBuffer_deinitlock(unsigned long core_idx)
{
	...
	SEM_DESTROY(singleEsBuf_sem[core_idx]);
}

void vdi_SingleEsBuffer_lock(unsigned long core_idx)
{
	int intr_reason = 0;
	long ltimeout = 1000;
	...
	intr_reason = sem_timedwait(&singleEsBuf_sem[core_idx], &ts);
	if (intr_reason < 0) {
		CVI_VC_ERR("sem_timedwait timeout\n");
	}
}

void vdi_SingleEsBuffer_unlock(unsigned long core_idx)
{
	CVI_VC_LOCK("core_idx:%ld\n", core_idx);
...
#endif /* ANDROID */
#else
	if (TRUE != xSemaphoreTake((SemaphoreHandle_t)vdi->vpu_disp_mutex, -1)) {
		CVI_VC_ERR("lock error\n");
		return -1;
	}
#endif
...
#else
	xSemaphoreGive((SemaphoreHandle_t)vdi->vpu_disp_mutex);
#endif
}
...
unsigned int vdi_read_register(unsigned long core_idx, unsigned long addr)
{
	...
	// maybe value is not equal reg_addr, return the reg_addr
	if (value != (*reg_addr)) {
		// printf("==========value:%d, reg_addr:%d\n", value, *reg_addr);
		return *reg_addr;
	}
		    ...
		    unsigned char *data, int len, int endian)
{
	...
	CVI_VC_TRACE(
			"i = %d, addr = 0x"VCODEC_UINT64_FORMAT_HEX", phys_addr = 0x"VCODEC_UINT64_FORMAT_HEX", virt_addr = 0x%p, data = 0x%p\n",
...
int vdi_allocate_ion_memory(unsigned long core_idx, vpu_buffer_t *vb, int is_cached, const char *str)
{
	...
	if (sys_ion_alloc((uint64_t *)&vb->phys_addr, (void **)&vb->virt_addr,
			(char *)str, vb->size, is_cached) != 0) {
		CVI_VC_ERR("[VDI] fail to allocate ion memory. size=%d\n",
			   vb->size);
		ret = -1;
		goto ALLOCATE_ION_ERROR;
	}
	memset(vb->virt_addr, 0x0, vb->size);
	vdb.phys_addr = vb->phys_addr;
	vdb.virt_addr = vb->virt_addr;
	...
	//CVI_VC_ERR("physaddr=%llx, virtaddr=%p~%p, size=0x%x, cached=%d\n",
	//	   vb->phys_addr, (void *)vb->virt_addr,
...
void vdi_free_ion_memory(unsigned long core_idx, vpu_buffer_t *vb)
{
	...
	//CVI_VC_ERR("physaddr=%llx, virtaddr=%p~%p, size=0x%x\n",
	...
	if (sys_ion_free((uint64_t)vb->phys_addr) != 0) {
		CVI_VC_ERR("[VDI] fail to free ion phys_addr = %p\n",
			   vb->virt_addr);
		goto FREE_ION_ERROR;
	}
...
int vdi_allocate_dma_memory(unsigned long core_idx, vpu_buffer_t *vb) // saw not lock
{
	...
	vdb.size = vb->size;
	vdb.base = (unsigned long)calloc(1, vdb.size + 64);
	if (vdb.base == -1) {
	...
	switch (vdi->chip_version)  {
		case CHIP_ID_1822:
			switch (vdi->product_code) {
				case CODA980_CODE:  return VDI_SRAM_SIZE_1822_CODA9;
				case WAVE420L_CODE: return VDI_SRAM_SIZE_1822_WAVE420L;
			}
			break;

		case CHIP_ID_1835:
			switch (vdi->product_code) {
				case CODA980_CODE:  return VDI_SRAM_SIZE_1835_CODA9;
				case WAVE420L_CODE: return VDI_SRAM_SIZE_1835_WAVE420L;
			}
			break;
	}
...
int vdi_wait_interrupt(unsigned long coreIdx, int timeout, uint64_t *pu64TimeStamp)
{
	...
	intr_reason = sem_timedwait(&vdi->irq_sem, &ts);
	if (intr_reason < 0)
		return -1;

	intr_reason = vdi->intr_reason;
	vdi->intr_reason = 0;
#else
...
void cvi_vc_clk_enable(void)
{
	...
	SYS_CLK_EN_2 |= (1<<12);//clk_prepare_enable(jdev->clk_jpeg);
	SYS_CLK_EN_2 |= (1<<13);//clk_prepare_enable(jdev->clk_apb_jpeg);
}

void cvi_vc_clk_disable(void)
{
	//disable h264/h265/jpg clk
	SYS_CLK_EN_2 &= ~(1<<10);//clk_disable_unprepare(vdev->clk_h264c);
	SYS_CLK_EN_2 &= ~(1<<14);//clk_disable_unprepare(vdev->clk_apb_h264c);
	...
	SYS_CLK_EN_2 &= ~(1<<12);//clk_disable_unprepare(jdev->clk_jpeg);
	SYS_CLK_EN_2 &= ~(1<<13);//clk_disable_unprepare(jdev->clk_apb_jpeg);

...
// #endif
	...
	//pBase = (Uint8*)calloc(1, USERDATA_BUFFER_SIZE);
...
// #endif
	...
	CVI_VC_INFO("-------------- DECODER OPTIONS --------------------\n");
	CVI_VC_INFO("[coreIdx            ]: %d\n", pdop->coreIdx);
	CVI_VC_INFO("[mapType            ]: %d\n", pdeccfg->mapType);
	CVI_VC_INFO("[tiled2linear       ]: %d\n",pdeccfg->coda9.enableTiled2Linear);
	CVI_VC_INFO("[wtlEnable          ]: %d\n", pdop->wtlEnable);
	CVI_VC_INFO("[wtlMode            ]: %d\n", pdop->wtlMode);
	CVI_VC_INFO("[bitstreamBuffer    ]: 0x%08lx\n", pdop->bitstreamBuffer);
	CVI_VC_INFO("[bitstreamBufferSize]: %d\n", pdop->bitstreamBufferSize);
	CVI_VC_INFO("[bitstreamMode      ]: %d\n", pdop->bitstreamMode);
	CVI_VC_INFO("[cbcrInterleave     ]: %d\n", pdop->cbcrInterleave);
	CVI_VC_INFO("[nv21               ]: %d\n", pdop->nv21);
	CVI_VC_INFO("[streamEndian       ]: %d\n", pdop->streamEndian);
	CVI_VC_INFO("[frameEndian        ]: %d\n", pdop->frameEndian);
	CVI_VC_INFO("[BWB                ]: %d\n", pdop->bwbEnable);
	CVI_VC_INFO("----------------------------------------------------\n");
...
int _cviH265_GetDecodedData(cviVideoDecoder *pvdec)
{
	...
	if (pvdec->sequenceChangeFlag) {
		...
		if (dpbChanged || sizeChanged || bitDepthChanged) {
			DecOutputInfo *pDecOutInfo = (DecOutputInfo *)calloc(1, sizeof(DecOutputInfo) * MAX_GDI_IDX);

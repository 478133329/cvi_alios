...
#endif
extern void rcLibUpdateRc(CodecInst *pCodecInst, EncOutputInfo *info, int avg_qp, int real_pic_bit);
...
static void GetSequenceInformation(CodecInst *instance, DecInitialInfo *info)
{
	...
	if (info->isExtSAR == TRUE) {
		info->aspectRateInfo = VpuReadReg(instance->coreIdx, W4_RET_DEC_ASPECT_RATIO);
...
RetCode Wave4VpuDecGetResult(CodecInst *instance, DecOutputInfo *result)
{
	...
	if (result->sequenceChanged == 0) {
		...
		memcpy((void *)&pDecInfo->newSeqInfo,
				       ...
				       TiledMapType mapType, Uint32 count)
{
	...
	Uint32 regVal = 0, cbcrInterleave = 0, nv21 = 0, lumaStride = 0, chromaStride = 0;
	...
	VpuWriteReg(coreIdx, W4_ADDR_SUB_SAMPLED_FB_BASE, pEncInfo->vbSubSamBuf.phys_addr);
...
RetCode Wave4VpuEncSetup(CodecInst *instance)
{
	...
	pEncInfo->streamWrPtr =
		vdi_remap_memory_address(coreIdx, VpuReadReg(coreIdx, pEncInfo->streamWrPtrRegAddr));
	return RETCODE_SUCCESS;
}
...
static RetCode Wave4SetEncBitStreamInfo(CodecInst *instance, EncHeaderParam *encHeaderParam, EncParam *option)
{
...
RetCode Wave4VpuEncode(CodecInst *instance, EncParam *option)
{
	...
	CVI_VC_SRC("bufY = 0x"VCODEC_UINT64_FORMAT_HEX", bufCb = 0x"VCODEC_UINT64_FORMAT_HEX", bufCr = 0x"VCODEC_UINT64_FORMAT_HEX"\n",
...
RetCode Wave4VpuEncGetResult(CodecInst *instance, EncOutputInfo *result)
{
	...
	CVI_VC_BS("bitstreamSize = 0x%X, bitstreamBuffer = 0x"VCODEC_UINT64_FORMAT_HEX"\n"
		  "rdPtr = 0x"VCODEC_UINT64_FORMAT_HEX", wrPtr = 0x"VCODEC_UINT64_FORMAT_HEX"\n",
...
static void printResult(EncInfo *pEncInfo, EncOutputInfo *result)
{
	...
	CVI_VC_FLOW("rdPtr = 0x"VCODEC_UINT64_FORMAT_HEX", wrPtr = 0x"VCODEC_UINT64_FORMAT_HEX", bitstreamSize = 0x%x\n",
...
RetCode Wave4VpuEncGetHeader(EncHandle instance, EncHeaderParam *encHeaderParam)
{
	...
	rdPtr = vdi_remap_memory_address(coreIdx, VpuReadReg(coreIdx, pEncInfo->streamRdPtrRegAddr));
	wrPtr = vdi_remap_memory_address(coreIdx, VpuReadReg(coreIdx, pEncInfo->streamWrPtrRegAddr));

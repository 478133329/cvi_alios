...
CVI_S32 vo_send_frame(VO_LAYER VoLayer, VO_CHN VoChn, VIDEO_FRAME_INFO_S *pstVideoFrame, CVI_S32 s32MilliSec)
{
	...
	if (ret != CVI_SUCCESS) {
		CVI_TRACE_VO(CVI_VO_ERR, "pstVideoFrame NULL pointer\n");
		return ret;
	}
...
CVI_S32 vo_send_logo_from_ion(VO_LAYER VoLayer, VO_CHN VoChn, VIDEO_FRAME_INFO_S *pstVideoFrame, CVI_S32 s32MilliSec)
{
	extern CVI_S32 _vo_send_logo_from_ion(VO_LAYER VoLayer, VO_CHN VoChn, VIDEO_FRAME_INFO_S *pstVideoFrame, CVI_S32 s32MilliSec);
	_vo_send_logo_from_ion(VoLayer, VoChn, pstVideoFrame, s32MilliSec);
	return CVI_SUCCESS;
}
		...
		struct vo_video_format_mplane *mp)
{
	...
	sclr_disp_get_crop_cfg_from_reg();
}
...
static int vo_set_videolayerattr_ex(VO_LAYER VoLayer, const VO_VIDEO_LAYER_ATTR_S *pstLayerAttr)
{
	struct sclr_rect rect;
	CVI_U16 rgb[3] = {0, 0, 0};
	CVI_S32 ret = CVI_FAILURE;
	...
	if (!VO_SUPPORT_FMT(pstLayerAttr->enPixFormat)) {
		CVI_TRACE_VO(CVI_VO_ERR, "VoLayer(%d) enPixFormat(%d) unsupported\n"
			, VoLayer, pstLayerAttr->enPixFormat);
		return CVI_ERR_VO_ILLEGAL_PARAM;
	}
	...
	if (gVoCtx->stPubAttr.enIntfType != VO_INTF_I80) {
		_vo_sdk_setfmt(pstLayerAttr->stDispRect.u32Width,
					pstLayerAttr->stDispRect.u32Height, pstLayerAttr->enPixFormat);
	}
...
long vo_sdk_ctrl(struct cvi_vo_dev *vdev, struct vo_ext_control *p)
{
	...
	case VO_SDK_SEND_LOGO_FROMION:
	{
		...
		rc = vo_send_logo_from_ion(cfg->VoLayer, cfg->VoChn, &cfg->stVideoFrame, cfg->s32MilliSec);
		break;
	}
	...
	case VO_SDK_SET_VIDEOLAYERATTR_EX:
	{
		struct vo_video_layer_attr_cfg *cfg, _cfg_;
		...
		rc = vo_set_videolayerattr_ex(cfg->VoLayer, &cfg->pstLayerAttr);

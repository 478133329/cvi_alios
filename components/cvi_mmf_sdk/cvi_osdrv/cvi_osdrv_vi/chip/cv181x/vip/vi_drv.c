...
int ispblk_dma_buf_get_size2(struct isp_ctx *ctx, int dmaid, u8 raw_num)
{
	...
	{ //rgbmap max size
		if (ctx->is_slice_buf_on) {
			...
			len = (((UPPER(ctx->isp_pipe_cfg[raw_num].crop.w, grd_size)) * 6 + 15) >> 4) << 4;
			num = UPPER(ctx->isp_pipe_cfg[raw_num].crop.h, grd_size);
		}
	...
	case ISP_BLK_ID_DMA_CTL37: //manr_iir_w
	{
		...
		len = (((UPPER(ctx->isp_pipe_cfg[raw_num].crop.w, w_bit) << 4) + 127) >> 7) << 4;
		num = UPPER(ctx->isp_pipe_cfg[raw_num].crop.h, h_bit);
...
void ispblk_rgbmap_dma_config(struct isp_ctx *ctx, enum cvi_isp_raw raw_num, int dmaid)
{
	...
	stride = ((((w + grid_size - 1) / grid_size) * 6 + 47) / 48 * 8 * grid_size) / grid_size * 6;
...
#define F_D (15)
static inline void _ispblk_lsc_cfg_update(struct isp_ctx *ctx, enum cvi_isp_raw raw_num)
{
	...
	if (step == 1) {
		...
		if (ctx->isp_pipe_cfg[err_raw_num].is_422_to_420) {
			...
			if (ctx->isp_pipe_cfg[err_raw_num].is_yuv_bypass_path) {
				...
				else if (err_raw_num == ISP_PRERAW_C)
		...
		vi_pr(VI_INFO, "ISP and vip_sys isp rst 0x%x pull up\n", sw_rst.raw);
	} else if (step == 5) {
		if (!ctx->isp_pipe_cfg[err_raw_num].is_422_to_420) {
...
void ispblk_slice_buf_config(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num, u8 en)
{
	...
	if (en && _is_fe_be_online(ctx)) {
		...
		slice_en.bits.SLICE_ENABLE_SUB_LEXP = en && ctx->is_3dnr_on;
		...
		if (ctx->isp_pipe_cfg[raw_num].is_hdr_on) {
			...
			slice_en.bits.SLICE_ENABLE_SUB_SEXP = en && ctx->is_3dnr_on;

...
void ispblk_aehist_config(struct isp_ctx *ctx, int blk_id, bool enable)
{
	...
	sub_window_w = ((ctx->img_width / num_x) >> 1) << 1;
	sub_window_h = ((ctx->img_height / num_y) >> 1) << 1;
	...
	ISP_WR_REG(sts, REG_ISP_AE_HIST_T, STS_AE_STS_DIV, (1 << 20) / (sub_window_w * sub_window_h / 4));
}

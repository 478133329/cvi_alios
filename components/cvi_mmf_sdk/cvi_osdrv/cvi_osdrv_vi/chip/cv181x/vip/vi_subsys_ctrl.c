...
void ispblk_yuvtop_out_config(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num, bool online2sc)
{
	uintptr_t yuvtop = ctx->phys_regs[ISP_BLK_ID_YUVTOP];

	ISP_WR_BITS(yuvtop, REG_YUV_TOP_T, YUV_CTRL, BYPASS_V, online2sc);
}
	...
	if (ctx->isp_pipe_cfg[ISP_PRERAW_A].is_offline_preraw)
		...
		if (!ctx->isp_pipe_cfg[ISP_PRERAW_A].is_yuv_bypass_path) { //RGB sensor
			if (ctx->isp_pipe_cfg[ISP_PRERAW_A].is_hdr_on &&
				!ctx->isp_pipe_cfg[ISP_PRERAW_A].is_synthetic_hdr_on)

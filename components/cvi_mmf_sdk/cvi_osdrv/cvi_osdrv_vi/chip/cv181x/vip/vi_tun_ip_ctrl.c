...
int vi_tuning_buf_setup(enum cvi_isp_raw raw_num)
{
	u8 i = raw_num;
	...
	const enum cvi_isp_raw raw_num)
{
	...
	reg_00.raw = ISP_RD_REG(ba, REG_ISP_CFA_T, REG_00);
	...
	ISP_WR_REG(ba, REG_ISP_CFA_T, REG_00, reg_00.raw);

	if (!cfg->cfa_enable)
		return;

	reg_04.raw = ISP_RD_REG(ba, REG_ISP_CFA_T, REG_04);
	reg_04.bits.CFA_OUT_SEL		= cfg->cfa_out_sel;
	reg_04.bits.CFA_EDGEE_THD2	= cfg->cfa_edgee_thd2;
	ISP_WR_REG(ba, REG_ISP_CFA_T, REG_04, reg_04.raw);

	ISP_WR_BITS(ba, REG_ISP_CFA_T, REG_20, CFA_RBSIG_LUMA_THD, cfg->cfa_rbsig_luma_thd);

	reg_110.raw = ISP_RD_REG(ba, REG_ISP_CFA_T, REG_110);
	reg_110.bits.CFA_YMOIRE_LPF_W	= cfg->cfa_ymoire_lpf_w;
	reg_110.bits.CFA_YMOIRE_DC_W	= cfg->cfa_ymoire_dc_w;
	ISP_WR_REG(ba, REG_ISP_CFA_T, REG_110, reg_110.raw);

	ISP_WR_REG_LOOP_SHFT(ba, REG_ISP_CFA_T, REG_30, 32, 4, cfg->cfa_ghp_lut, 8);

	ISP_WR_REGS_BURST(ba, REG_ISP_CFA_T, REG_0C, cfg->demosiac_cfg, cfg->demosiac_cfg.REG_0C);

	ISP_WR_REGS_BURST(ba, REG_ISP_CFA_T, REG_120, cfg->demosiac_1_cfg, cfg->demosiac_1_cfg.REG_120);

	ISP_WR_REGS_BURST(ba, REG_ISP_CFA_T, REG_90, cfg->demosiac_2_cfg, cfg->demosiac_2_cfg.REG_90);
}
...
{
	uintptr_t tnr = ctx->phys_regs[ISP_BLK_ID_TNR];
	static uint8_t mono_enable[ISP_PRERAW_MAX] = {0};

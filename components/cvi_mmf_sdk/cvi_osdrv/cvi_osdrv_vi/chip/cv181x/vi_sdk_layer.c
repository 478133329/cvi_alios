...
static int _vi_sdk_drv_qbuf(struct cvi_buffer *buf, CVI_S32 chnId)
{
	...
	if (qbuf == NULL) {
		vi_pr(VI_ERR, "qbuf kzalloc size(%lu) failed\n", sizeof(struct cvi_isp_buf2));
		return -ENOMEM;
	}
...
{
	VB_BLK blk = vb_get_block_with_id(VB_INVALID_POOLID, gViCtx->blk_size[chn.s32ChnId], CVI_ID_VI);
	SIZE_S size = gViCtx->chnAttr[chn.s32ChnId].stSize;
	int rc = CVI_SUCCESS;

	if (blk == VB_INVALID_HANDLE) {
		gViCtx->chnStatus[chn.s32ChnId].u32VbFail++;
		vi_pr(VI_INFO, "Can't acquire VB BLK for VI, size(%d)\n", gViCtx->blk_size[chn.s32ChnId]);
		return -ENOMEM;
	}
...
{
	if (mlv_i.sensor_num < 0 || mlv_i.sensor_num >= VI_MAX_DEV_NUM)
		return CVI_FAILURE;
...
CVI_S32 vi_set_dev_attr(VI_DEV ViDev, const VI_DEV_ATTR_S *pstDevAttr)
{
	...
	if (pstDevAttr->enInputDataType == VI_DATA_TYPE_YUV) {
		...
		gvdev->snr_info[ViDev].raw_num = ViDev;
		gvdev->snr_info[ViDev].color_mode = 0;
		gvdev->snr_info[ViDev].snr_fmt.frm_num = 1;
		gvdev->snr_info[ViDev].snr_fmt.img_size[0].max_width =
			gvdev->snr_info[ViDev].snr_fmt.img_size[0].active_w =
			gvdev->snr_info[ViDev].snr_fmt.img_size[0].width = pstDevAttr->stSize.u32Width;
		gvdev->snr_info[ViDev].snr_fmt.img_size[0].max_height =
			gvdev->snr_info[ViDev].snr_fmt.img_size[0].active_h =
			gvdev->snr_info[ViDev].snr_fmt.img_size[0].height = pstDevAttr->stSize.u32Height;
		gvdev->snr_info[ViDev].snr_fmt.img_size[0].start_x = 0;
		gvdev->snr_info[ViDev].snr_fmt.img_size[0].start_y = 0;
	}
	...
	if ((pstDevAttr->stWDRAttr.enWDRMode == WDR_MODE_2To1_LINE) ||
		...
		gvdev->ctx.isp_pipe_cfg[ViDev].is_synthetic_hdr_on = pstDevAttr->stWDRAttr.bSyntheticWDR;
...
CVI_S32 vi_enable_dev(VI_DEV ViDev)
{
	...
	if ((ViDev < (VI_DEV)ISP_PRERAW_A) || (ViDev >= (VI_DEV)ISP_PRERAW_MAX)) {
		vi_pr(VI_ERR, "vi_enable_dev invalid Dev(%d)\n", ViDev);
		return CVI_FAILURE;
	}

	if (gViCtx->isDevEnable[ViDev])
		return CVI_SUCCESS;

	//if pipeline is_running, just init status
	if (atomic_read(&gvdev->isp_streamon) == 1) {
	...
	gViCtx->enVcWdrMode[ViDev] = gViCtx->devAttr[ViDev].enVcWdrMode;
	gViCtx->isDevEnable[ViDev] = true;
	gViCtx->total_dev_num++;
	gViCtx->bypass_frm[ViDev] = START_SKIP_FRAME;
...
CVI_S32 vi_disable_chn(VI_CHN ViChn)
{
	...
	for (i = 0; i < gViCtx->total_chn_num; i++) {
		...
		if (ViChn == (gViCtx->total_chn_num - 1)) {
			...
			vi_destory_thread(gvdev, E_VI_TH_EVENT_HANDLER);
			//TODO maybe can move to vi_event_handler
...
CVI_S32 vi_get_chn_frame(VI_PIPE ViPipe, VI_CHN ViChn, VIDEO_FRAME_INFO_S *pstFrameInfo, CVI_S32 s32MilliSec)
{
	...
	vi_pr(VI_DBG, "pixfmt(%d), w(%d), h(%d), pts(%"PRIx64"), addr(0x%"PRIx64", 0x%"PRIx64", 0x%"PRIx64")\n",
...
CVI_S32 vi_release_chn_frame(VI_PIPE ViPipe, VI_CHN ViChn, VIDEO_FRAME_INFO_S *pstFrameInfo)
{
	...
	if (blk == VB_INVALID_HANDLE) {
		vi_pr(VI_ERR, "Invalid phy-address(%"PRIx64") in pstVideoFrame. Can't find VB_BLK.\n"
	...
	vi_pr(VI_DBG, "release chn frame, addr(0x%"PRIx64")\n",
			pstFrameInfo->stVFrame.u64PhyAddr[0]);
...
{
	for (CVI_S8 i = 0; i < ISP_PRERAW_MAX; i++)
		free_isp_byr(i);
...
CVI_S32 vi_get_smooth_rawdump(VI_PIPE ViPipe, VIDEO_FRAME_INFO_S *pstFrameInfo, CVI_S32 s32MilliSec)
{
	...
	for (; i < frm_num; i++) {
		...
		vi_pr(VI_DBG, "Get paddr(0x%"PRIx64") size(%d) frm_num(%d)\n",

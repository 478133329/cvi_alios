...
static inline CVI_S32 S_CTRL_PTR(int _fd, void *_cfg, int _ioctl, CVI_U32 _handle)
{
	...
	if (rgn_ioctl(RGN_IOC_S_CTRL, (unsigned long)&ec1) < 0) {
...
static inline CVI_S32 SDK_CTRL_GET_CFG(int _fd, void *_cfg1, void *_cfg2, int _ioctl, CVI_U32 _handle)
{
	...
	if (rgn_ioctl(RGN_IOC_G_CTRL, (unsigned long)&ec1) < 0) {
...
int rgn_get_ctx(int fd, int Handle, struct cvi_rgn_ctx *pCtx)
{
	return SDK_CTRL_GET_CFG(fd, (void *)pCtx, NULL, RGN_SDK_GET_CTX, Handle);
}

int rgn_get_ctx_pointer(int fd, int Handle, struct cvi_rgn_ctx **ppCtx)
{
	return SDK_CTRL_GET_CFG(fd, (void *)ppCtx, NULL, RGN_SDK_GET_CTX_POINTER, Handle);
}

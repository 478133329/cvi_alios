...
#define VO_S_CTRL_VALUE(_fd, _cfg, _ioctl)\
	do {\
		...
		ec1.id = _ioctl;\
		ec1.value = _cfg;\
		if (vo_ioctl(VO_IOC_S_CTRL, (unsigned long)&ec1) < 0) {\
...
#define VO_G_CTRL_VALUE(_fd, _out, _ioctl)\
	do {\
		...
		ec1.id = _ioctl;\
		ec1.value = 0;\
		if (vo_ioctl(VO_IOC_G_CTRL, (unsigned long)&ec1) < 0) {\
...
int vo_sdk_send_logo_fromIon(int fd, struct vo_snd_frm_cfg *cfg)
{
	VO_SDK_CTRL_PTR(fd, cfg, VO_IOCTL_SDK_CTRL, VO_SDK_SEND_LOGO_FROMION);
}
...
int vo_sdk_set_videolayerattr_ex(int fd, struct vo_video_layer_attr_cfg *cfg)
{
	VO_SDK_CTRL_PTR(fd, cfg, VO_IOCTL_SDK_CTRL, VO_SDK_SET_VIDEOLAYERATTR_EX);
}

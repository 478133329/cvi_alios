	...
	const TISPD2HandlerInfo *ptHandlerInfo, TISPDeviceInfo *ptDeviceInfo)
{
	...
	do {
		...
		//	ISPD2_json_object_to_json_string_ext(pJsonObj, JSON_C_TO_STRING_SPACED));
		...
		// printf("parse offset start : %d, len : %d\n", u32ReadOffset, u32ParseOffset);
		u32ReadOffset += u32ParseOffset;
...
// -----------------------------------------------------------------------------
EMessageFormat CVI_ISPD2_Message_CommandChecker(const char *pszBuf, CVI_U32 u32CheckStart, CVI_U32 u32CheckTail)
{
	if ((isascii(*(pszBuf + u32CheckStart)) == 0) || (isascii(*(pszBuf + u32CheckTail)) == 0)) {
		return EMESSAGE_FORMAT_BINARY;
	}

	const char	*pcHeadChar;
	const char	*pcTailChar;
	CVI_U32		u32HeadPos, u32TailPos;

	u32HeadPos = u32CheckStart;
	pcHeadChar = pszBuf + u32HeadPos;
	while (isspace((CVI_U8)*pcHeadChar) && (u32HeadPos < u32CheckTail)) {
		pcHeadChar++;
		u32HeadPos++;
	}

	u32TailPos = u32CheckTail;
	pcTailChar = pszBuf + u32TailPos;
	while (isspace((CVI_U8)*pcTailChar) && (u32TailPos > u32CheckStart)) {
		pcTailChar--;
		u32TailPos--;
	}

	// ISP_DAEMON2_DEBUG_EX(LOG_DEBUG, "Head %d/(%d~%d), %c\nTail %d/(%d~%d), %c",
	//	u32HeadPos, u32CheckStart, u32CheckTail, *pcHeadChar,
	//	u32TailPos, u32CheckStart, u32CheckTail, *pcTailChar);

	EMessageFormat eRetEType = EMESSAGE_FORMAT_JSONRPC_PARTICAL;

	if (u32HeadPos >= u32TailPos) {
		return eRetEType;
	}

	char cBufHead = *(pcHeadChar);
	char cBufTail = *(pcTailChar);

	if (cBufHead == JSONRPC_FLAG_ARRAY_HEAD) {
		eRetEType |= EMESSAGE_FORMAT_JSONRPC_ARRAY_HEAD;
	}
	if (cBufTail == JSONRPC_FLAG_ARRAY_TAIL) {
		eRetEType |= EMESSAGE_FORMAT_JSONRPC_ARRAY_TAIL;
	}

	if (cBufHead == JSONRPC_FLAG_ELEMENT_HEAD) {
		eRetEType |= EMESSAGE_FORMAT_JSONRPC_ELEMENT_HEAD;
	}
	if (cBufTail == JSONRPC_FLAG_ELEMENT_TAIL) {
		eRetEType |= EMESSAGE_FORMAT_JSONRPC_ELEMENT_TAIL;
	}

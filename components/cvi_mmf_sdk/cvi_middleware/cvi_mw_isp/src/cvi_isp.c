...
ISP_FUNC_FROM_OUT_S gst_isp_fc_from_out;
...
{
	 gst_isp_fc_from_out = stFunction;
...
static CVI_S32 handle_fifo_command(const char *command)
{
	...
	if (strcmp(TRIGGER_AE_LOG_CMD, command) == 0) {
		...
		for (CVI_U32 u32PipeID = 0; u32PipeID < VI_MAX_PIPE_NUM; ++u32PipeID) {
			isp_log_ctrl_ctrl(u32PipeID, &tLogOpts);
		}
...
CVI_S32 _ISP_Run(VI_PIPE ViPipe)
{
	...
	while (g_isp_thread_run) {
		...
		if(get_isp_check_event() < 0) {//queue empty
			...
			ret = aos_event_get(isp_attr.event, 0x01, AOS_EVENT_OR_CLEAR, &actl_flags, SEM_WAIT_TIMEOUT_MS);
			if (ret != 0) {
				//printf("time out\n");
				continue;
			}
		...
		intervaltime = (ts.tv_sec * 1000 + ts.tv_nsec/1000000) - (ev.timestamp.tv_sec * 1000 +
						ev.timestamp.tv_nsec/1000000);
		...
		{
			ISP_LOG_INFO("VI_EVENT_ISP_PROC_READ\n");
...
static CVI_VOID *ISP_Thread(void *arg)
{
	...
	ISP_LOG_INFO("ISP Dev %d running!\n", IspDev);
	s32Ret = _ISP_Run(IspDev);
	if (s32Ret != 0)
		ISP_LOG_ERR("CVI_ISP_Run failed with %#x!\n", s32Ret);
...
CVI_S32 CVI_ISP_Run(VI_PIPE ViPipe)
{
	...
	if (arg == NULL) {
		ISP_LOG_ERR("malloc failed\n");
		goto out;
	}

	*arg = ViPipe;
	param.sched_priority = CVI_MW_TASK_PRIO;
	...
	if (s32Ret != 0) {
		ISP_LOG_ERR("create isp running thread failed!, error: %d, %s\r\n",
...
CVI_S32 CVI_ISP_Exit(VI_PIPE ViPipe)
{
	...
	for (CVI_S32 i = 0; i < VI_MAX_DEV_NUM; i++) {
		if (g_IspPid[i]) {
...
CVI_S32 CVI_ISP_MediaVideoInit(CVI_BOOL IsRawReplayMode)
{
	...
	if (gst_isp_fc_from_out.pfnMediaVideoInit) {
		s32Ret = gst_isp_fc_from_out.pfnMediaVideoInit(IsRawReplayMode);
	}
	else {
		ISP_LOG_WARNING("pfnMediaVideoInit is null\n");
		s32Ret = CVI_FAILURE;
	}
	return s32Ret;
}
...
{
	CVI_S32 s32Ret = CVI_SUCCESS;
	if (gst_isp_fc_from_out.pfnMediaVideoDeinit) {
		s32Ret = gst_isp_fc_from_out.pfnMediaVideoDeinit(IsRawReplayMode);
	}
	else {
		ISP_LOG_WARNING("pfnMediaVideoDeinit is null\n");
		s32Ret = CVI_FAILURE;
	}
	UNUSED(IsRawReplayMode);
	return s32Ret;
}
...
{
	if (aos_get_mminfo_resv(ptotal, pused, pmfree, ppeak) != 0) {
		return CVI_FAILURE;
	}
	return CVI_SUCCESS;
}

...
#define ENV_PROC_LEVEL		"PROC_LEVEL"
	...
	if ((Level > PROC_LEVEL_3) || (pszEnv == pszEnd)) {
		ISP_LOG_INFO("ENV_PROC_LEVEL invalid\n");
		pstIspProcCtrl->u32ProcLevel = PROC_LEVEL_0;
		return CVI_FAILURE;
	}
...
#define ENV_PROC_RATE "PROC_PARAM"
	...
	if (pszEnv == pszEnd) {
		ISP_LOG_INFO("ENV_PROC_RATE invalid\n");
		pstIspProcCtrl->u32ProcParam = 30;
		return CVI_FAILURE;
	}
...
{
	ISP_LOG_INFO("g_pIspTempProcST[%d]:%u g_pIspProcST[%d]:%u\n",
...
CVI_S32 isp_proc_writeInfoToFile(void)
{
	...
	for (CVI_U8 ViPipe = 0; ViPipe < VI_MAX_PIPE_NUM; ViPipe++) {
		...
		if (g_bIsOpenPipe[ViPipe]) {
			...
			ISP_LOG_INFO("Start write pipe[%d] loglevel[%d]\n",
	...
	ISP_LOG_INFO("moduleTotalSize:%d\n", moduleTotalSize);
...
void *ISP_ProcThread(void *param)
{
	...
	while (1) {
		...
		if (!g_thread_run)
			break;
		ISP_LOG_INFO("receive proc read event\n");

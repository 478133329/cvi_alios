...
#define ISP_VERSION "-"
#endif
	ISP_LOG_INFO("ISP VERSION(%s)\n", ISP_VERSION);

	VI_DEV_ATTR_S stViDevAttr;
	CVI_S32 ret = CVI_SUCCESS;

	memset(&stViDevAttr, 0, sizeof(VI_DEV_ATTR_S));
	ret = CVI_VI_GetDevAttr(ViPipe, &stViDevAttr);
	if (ret != CVI_SUCCESS) {
		ISP_LOG_ERR("CVI_VI_GetDevAttr failed!\n");
	}
...
#define ENV_DEBUG_MESSAGE_TO_STDOUT		"ISP_ENABLE_STDOUT_DEBUG"
	...
	if (pszEnv == pszEnd) {
		ISP_LOG_INFO("ISP_ENABLE_STDOUT_DEBUG invalid\n");
		return CVI_FAILURE;
	}
...
#define ENV_DEBUG_LEVEL		"ISP_DEBUG_LEVEL"
	...
	if ((ulLevel > LOG_DEBUG) || (pszEnv == pszEnd)) {
		ISP_LOG_INFO("ISP_DEBUG_LEVEL invalid\n");
		return CVI_FAILURE;
	}
...
static CVI_S32 unset_sensorname_to_environment(VI_PIPE ViPipe)
{
	...
	if (ret != 0) {
		ISP_DEBUG(LOG_WARNING, "unset env value:%s fail.\n", sensorNameEnv);
	}

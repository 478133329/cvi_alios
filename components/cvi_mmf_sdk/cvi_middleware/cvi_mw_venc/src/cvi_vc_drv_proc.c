...
// TODO: following info should be amended later
				...
				printf(
					"-----VENC PTS STATE------------------------------------------------\n");
				printf(
					"ID: %d\t RcvFirstFrmPts: %llu\t RcvFrmPts: %lu\n",
					idx, 0LL, pstVFrame->u64PTS);

				printf(
					"-----VENC CHN PERFORMANCE------------------------------------------------\n");
				printf(
					"ID: %d\t No.SendFramePerSec: %u\t No.EncFramePerSec: %u\t HwEncTime: %lu us\n\n",
					idx, pstFPS->u32InFPS,
		...
		for (idx = 0; idx < MaxVdecChnNum; idx++) {
			if (vdec_handle &&
				...
				printf(
					...
					pstFrame->u32Height);
				printf(
					"\t Stride: %u\t PixelFormat: %s\t PTS: %lu\t PA: 0x%lx\n",
				...
				printf(
					"ID: %d\t No.SendStreamPerSec: %u\t No.DecFramePerSec: %u\t HwDecTime: %lu us\n\n",
...
static int codec_proc_show()
{
	...
	VENC_DATA_FIFO_CTX_S *pstDataFifoCtx;;
	...
	for (idx = 0; idx < MaxVencChnNum; idx++) {
		...
		if (pChnHandle != NULL) {
			...
			pstDataFifoCtx = pChnHandle->pstDataFifoCtx;
			if (pstDataFifoCtx)
				printf("DataFifoDropCnt:%d    WaitqDropCnt:%d\n",
				pstDataFifoCtx->u32DropCnt, venc_vb_ctx[idx].waitq_dropcount);
		}
	}
...
ALIOS_CLI_CMD_REGISTER(vcodec_venc_proc_show, proc_venc, venc info);
ALIOS_CLI_CMD_REGISTER(vcodec_h265e_proc_show, proc_h265e, h265 encode info);
ALIOS_CLI_CMD_REGISTER(vcodec_h264e_proc_show, proc_h264e, h264 encode info);
ALIOS_CLI_CMD_REGISTER(vcodec_jpege_proc_show, proc_jpege, jpeg encode info);
ALIOS_CLI_CMD_REGISTER(vcodec_rc_proc_show, proc_rc, rc info);
ALIOS_CLI_CMD_REGISTER(vcodec_vdec_proc_show, proc_vdec, vdec info);
ALIOS_CLI_CMD_REGISTER(vcodec_codec_proc_show, proc_codec, codec info);

...
static CVI_S32 jpege_enc_one_pic(CVI_VOID *ctx, const VIDEO_FRAME_INFO_S *pstFrame, int s32MilliSec);
static CVI_VOID setSrcInfo(CVIFRAMEBUF *psi, venc_enc_ctx *pEncCtx, const VIDEO_FRAME_INFO_S *pstFrame);
static CVI_S32 jpege_get_stream(CVI_VOID *ctx, VENC_STREAM_S *pstStream, CVI_S32 s32MilliSec);
...
static CVI_S32 vidEnc_enc_one_pic(CVI_VOID *ctx, const VIDEO_FRAME_INFO_S *pstFrame, CVI_S32 s32MilliSec);
static CVI_S32 vidEnc_get_stream(CVI_VOID *ctx, VENC_STREAM_S *pstStream, CVI_S32 s32MilliSec);
...
static CVI_VOID h265e_setInitCfgFixQp(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h265e_setInitCfgCbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h265e_setInitCfgVbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h265e_setInitCfgAVbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h265e_setInitCfgQpMap(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h265e_setInitCfgUbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
...
static CVI_VOID cviSetInitCfgGop(cviInitEncConfig *pInitEncCfg, venc_chn_context *pChnHandle);
static CVI_VOID h264e_setInitCfgFixQp(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h264e_setInitCfgCbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h264e_setInitCfgVbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h264e_setInitCfgAVbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
static CVI_VOID h264e_setInitCfgUbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx);
...
static CVI_S32 jpege_open(CVI_VOID *handle, CVI_VOID *pchnctx)
{
	...
	if (config.u.enc.singleEsBuffer)
		config.u.enc.bitstreamBufSize = pHandle->ModParam.stJpegeModParam.u32SingleEsBufSize;
	else
...
static CVI_S32 jpege_enc_one_pic(CVI_VOID *ctx, const VIDEO_FRAME_INFO_S *pstFrame, CVI_S32 s32MIlliSec)
{
	CVIJpgHandle *pHandle;
	...
	status = CVIJpgSendFrameData(pHandle, (void *)&srcInfo, JPEG_MEM_EXTERNAL, s32MIlliSec);
...
static CVI_VOID setSrcInfo(CVIFRAMEBUF *psi, venc_enc_ctx *pEncCtx, const VIDEO_FRAME_INFO_S *pstFrame)
{
	...
	psi->vbY.phys_addr = pstVFrame->u64PhyAddr[0] + psi->strideY * pvecb->y + pvecb->x;
	...
	case PIXEL_FORMAT_YUV_PLANAR_422:
		...
		psi->vbCb.phys_addr = pstVFrame->u64PhyAddr[1] + psi->strideC * pvecb->y + pvecb->x / 2;
		psi->vbCr.phys_addr = pstVFrame->u64PhyAddr[2] + psi->strideC * pvecb->y + pvecb->x / 2;
		break;
	...
	case PIXEL_FORMAT_NV21:
		...
		psi->chromaInterleave = (pstVFrame->enPixelFormat == PIXEL_FORMAT_NV12) ?
				      CVI_CBCR_INTERLEAVE :
				      CVI_CRCB_INTERLEAVE;
		psi->vbCb.phys_addr = pstVFrame->u64PhyAddr[1] + psi->strideC * pvecb->y / 2 + pvecb->x;
	...
	default:
		...
		psi->packedFormat = CVI_PACKED_FORMAT_NONE;
		psi->chromaInterleave = CVI_CBCR_SEPARATED;
		psi->vbCb.phys_addr = pstVFrame->u64PhyAddr[1] + psi->strideC * pvecb->y / 2 + pvecb->x / 2;
		psi->vbCr.phys_addr = pstVFrame->u64PhyAddr[2] + psi->strideC * pvecb->y / 2 + pvecb->x / 2;
		break;
	}
...
static CVI_S32 jpege_get_stream(CVI_VOID *ctx, VENC_STREAM_S *pstStream, CVI_S32 s32MilliSec)
{
	...
	status = CVIJpgGetFrameData(
		pHandle, (CVI_U8 *)&cviBuf, sizeof(CVIBUF), (unsigned long int *)&pEncCtx->base.u64EncHwTime);
...
static CVI_S32 vidEnc_open(CVI_VOID *handle, CVI_VOID *pchnctx)
{
	...
	if (pInitEncCfg->codec == CODEC_H264) {
		pInitEncCfg->userDataMaxLength = pHandle->ModParam.stH264eModParam.u32UserDataMaxLen;
		...
		pInitEncCfg->userDataMaxLength = pHandle->ModParam.stH265eModParam.u32UserDataMaxLen;
	...
	if (pInitEncCfg->bSingleEsBuf) {
		if (pInitEncCfg->codec == CODEC_H264)
			pInitEncCfg->bitstreamBufferSize = pHandle->ModParam.stH264eModParam.u32SingleEsBufSize;
		else if (pInitEncCfg->codec == CODEC_H265)
			pInitEncCfg->bitstreamBufferSize = pHandle->ModParam.stH265eModParam.u32SingleEsBufSize;
	} else {
	...
	CVI_VENC_TRACE("width = %d, height = %d, rcMode = %d\n",
		pInitEncCfg->width, pInitEncCfg->height, pInitEncCfg->rcMode);
...
static CVI_VOID cviSetInitCfgGop(cviInitEncConfig *pInitEncCfg, venc_chn_context *pChnHandle)
{
	...
	if (pga->enGopMode == VENC_GOPMODE_NORMALP) {
		...
		CVI_VENC_CFG("virtualIPeriod = %d, gop = %d, bgQpDelta = %d, ViQpDelta = %d\n",
			pInitEncCfg->virtualIPeriod, pInitEncCfg->gop,
...
static CVI_VOID h264e_setInitCfgFixQp(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx)
{
	...
	CVI_VENC_CFG("iqp = %d, pqp = %d, gop = %d, framerate = %d\n",
		     pInitEncCfg->iqp, pInitEncCfg->pqp, pInitEncCfg->gop, pInitEncCfg->framerate);
...
static CVI_VOID h264e_setInitCfgCbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx)
{
	...
	CVI_VENC_CFG("statTime = %d, gop = %d, bitrate = %d, fps = %d\n",
		     pInitEncCfg->statTime, pInitEncCfg->gop, pInitEncCfg->bitrate, pInitEncCfg->framerate);
...
static CVI_VOID h264e_setInitCfgVbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx)
{
	...
	CVI_VENC_CFG("statTime = %d, gop = %d, maxbitrate = %d, fps = %d, s32ChangePos = %d\n",
...
static CVI_VOID h264e_setInitCfgAVbr(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx)
{
	...
	CVI_VENC_CFG("StillPercent = %d, StillQP = %d, MotionSensitivity = %d\n",
		pInitEncCfg->s32MinStillPercent, pInitEncCfg->u32MaxStillQP, pInitEncCfg->u32MotionSensitivity);
	CVI_VENC_CFG("FrmLostOpen = %d, FrmGap = %d, PureStillThr = %d\n",
		pInitEncCfg->s32AvbrFrmLostOpen, pInitEncCfg->s32AvbrFrmGap, pInitEncCfg->s32AvbrPureStillThr);
...
static CVI_VOID h265e_setInitCfgQpMap(cviInitEncConfig *pInitEncCfg, CVI_VOID *pchnctx)
{
	...
	pInitEncCfg->bitrate = DEAULT_QP_MAP_BITRATE; // QpMap uses CBR as basic settings

...
typedef struct _SENSOR_INFO {
	CVI_U8 num;
	CVI_CHAR name[VI_MAX_PIPE_NUM][SENSORNAME_SIZE];
} SENSOR_INFO;
...
CVI_S32 header_bin_GetHeaderDescInfo(CVI_CHAR *pOutDesc, enum CVI_BIN_CREATMODE inCreatMode)
{
	...
	pOutDesc += SENSORNAME_SIZE * VI_MAX_PIPE_NUM;
...
static CVI_S32 isp_bin_checkMd5(VI_PIPE ViPipe, CVI_U8 *addr, CVI_U32 binSize)
{
	...
	CVI_CHAR binMd5[BIN_MD5_SIZE] = { 0 };
	if(isp_bin_is_single_mode() && (ViPipe > 0)) {
		ViPipe = 0;
	}
	CVI_BIN_HEADER *header = (CVI_BIN_HEADER *)(addr - (binSize * ViPipe) - sizeof(CVI_BIN_HEADER));
...
static CVI_S32 isp_bin_checkBinVersion(CVI_U8 *addr, CVI_U32 binSize)
{
	...
	CVI_CHAR sensorName[VI_MAX_PIPE_NUM][SENSORNAME_SIZE];
	CVI_CHAR binCommitId[BIN_COMMIT_SIZE] = {0};
	CVI_CHAR binGerrit[BIN_GERRIT_SIZE] = {0};
	CVI_CHAR binMd5[BIN_MD5_SIZE] = { 0 };
	...
	memset(sensorName, 0, SENSORNAME_SIZE * VI_MAX_PIPE_NUM);
	...
	pDesc += SENSORNAME_SIZE * VI_MAX_PIPE_NUM;

...
static CVI_U16  tol_dig_gain;

static CVI_U16  gain_table_size_temp = gain_table_size;
...
static CVI_S32 cmos_again_calc_table(VI_PIPE ViPipe, CVI_U32 *pu32AgainLin, CVI_U32 *pu32AgainDb)
{
	...
	{
		gain_table_size_temp = gain_table_size_temp - 1;
		if(gain_table_size_temp < 12)
			gain_table_size_temp = 12;  //最低模拟增益档位
	}

	if(temperature_value <0x50)
			gain_table_size_temp = gain_table_size;

  if( gain_table[gain_table_size_temp]<(*pu32AgainLin) )
         *pu32AgainLin=gain_table[gain_table_size_temp];

	for (i = 1; i < gain_table_size_temp; i++) {
...
#endif
	tol_dig_gain = (*pu32AgainLin)*64/gain_table[i];

...
int MEDIA_VIDEO_ViInit(PARAM_VI_CFG_S * pstViCfg)
{
    ...
    for (int i = 0; i < devNum; i++) {
        ...
        if (pstViCfg->pstDevInfo) {
            stViDevAttr[ViDev].fbmEnable = pstViCfg->pstDevInfo[ViDev].s8FbmMode;
        }
    ...
    for (int i = 0; i < devNum; i++) {
        ...
        if (pstViCfg->pstIspCfg[i].s8ByPassNum != 0) {
            CVI_ISP_SetBypassFrm(i, pstViCfg->pstIspCfg[i].s8ByPassNum);// set by pass frm
        }
        MEDIA_CHECK_RET(start_isp(stPubAttr, stISP_FunctionFromOut, i),"start_isp fail");
    }
...
int MEDIA_VIDEO_VencChnInit(PARAM_VENC_CFG_S *pstVencCfg,int VencChn)
{
    ...
    switch (pstVecncChnCtx->stChnParam.u16EnType) {
        case PT_H264: {
            ...
            stAttr.stVencAttr.stAttrH264e.bSingleLumaBuf = 1;
            break;
        }
...
int MEDIA_VIDEO_Init(CVI_BOOL isRawReplayMode)
{
    ...
    if (!isRawReplayMode) {//calling sys init after deinit when raw replay
